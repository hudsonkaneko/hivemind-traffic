...
def test_observation_normalizer_round_trip():
    ...
    samples = np.array([[0.0] * 9, [1.0] * 9], dtype=np.float32)
...
def test_lane_action_masks_respect_road_boundaries():
    observations = np.zeros((2, 9), dtype=np.float32)
    ...
    torch.save(
        {
            ...
            "config": {"model": {"hidden_sizes": [16, 16]}},
        },
        checkpoint,
    )
    policy = CheckpointPolicy(checkpoint)
    observations = {"agent_0": np.zeros(9, dtype=np.float32)}
